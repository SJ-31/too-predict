...
metric_fns = {
    ...
    "consensus_score": met.consensus_score,
    "mutual_info_score": met.mutual_info_score,
}
...
def get_transformed(test):
    ...
    filter = fil.Filter(features=FEAT["edgeR_median_lfc_feature_list_3000"])
    adata = filter.transform(adata)
    clr = tt.Transformer("clr", "plus_one")
    tmm = tt.Transformer("tmm", "plus_one")
    clr_transformed = clr.fit_transform(adata)
...
if __name__ == "__main__":
    args = parse_args()
    clr_transformed, tmm_transformed = get_transformed(args["test"])
    # Compare clusterings
    result = {"metric": [], "clustering": [], "value": []}
    for method, kwargs in zip(
        [KMeans, AgglomerativeClustering], [{}, {"n_clusters": 8}]
    ):
        clr_clst = method(**kwargs).fit_predict(clr_transformed)
        tmm_clst = method(**kwargs).fit_predict(tmm_transformed)
        result["clustering"].append(method.__name__)
        for metric, fn in metric_fns.items():
            result["metric"].append(metric)
            result["value"].append(fn(clr_clst, tmm_clst))
    df = pd.DataFrame(result)
    df.to_csv(outdir.joinpath("clustering_comparison.csv"), index=False)

    n_features = clr_transformed.shape[1]
    clr_arr = StandardScaler().fit_transform(ut.xarray_if_sparse(clr_transformed))
    ...
    test_results = [
        ...
        for x, y in zip(clr_arr.T, tmm_arr.T)
    ]
    ...
    var.to_csv(outdir.joinpath("feature_results.csv"), index=False)
